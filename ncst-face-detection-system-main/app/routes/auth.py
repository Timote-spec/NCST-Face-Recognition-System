...
from app.email_service import send_otp_email, send_registration_verification_email
...
def _is_allowed_registration_email(email: str) -> bool:
    return email == settings.allowed_registration_email.strip().lower()
...
@router.post("/auth/register/request-verification", response_model=GenericResponse)
def request_registration_verification(body: RequestRegistrationOtpRequest):
    ...
    if not can_request_otp(email, OTP_PURPOSE_REGISTRATION):
        raise HTTPException(
            status_code=429,
            detail="Please wait a minute before requesting another verification code",
        )

    admin_email = settings.main_admin_email.strip().lower()
    otp = generate_otp()
    store_otp(email, otp, OTP_PURPOSE_REGISTRATION)
    _send_email_or_fail(
    ...
    log_system_action(
        email,
        "REGISTRATION_OTP",
        f"Registration verification sent to main admin ({admin_email}) for {email}",
    )

    return GenericResponse(
        status="ok",
        message="A verification code has been sent to the main administrator for approval",
    )
...
def register_admin(body: AdminRegisterRequest):
    ...
    if not verify_otp(email, body.otp.strip(), OTP_PURPOSE_REGISTRATION):
    ...
    log_system_action(email, "REGISTER", f"Admin account created for {email}")
...
def request_password_otp(body: RequestOtpRequest):
    ...
    if not can_request_otp(email, OTP_PURPOSE_PASSWORD_RESET):
        raise HTTPException(
            status_code=429,
    ...
    otp = generate_otp()
    store_otp(email, otp, OTP_PURPOSE_PASSWORD_RESET)
...
def reset_password_with_otp(body: ResetPasswordRequest):
    ...
    if not verify_otp(email, body.otp.strip(), OTP_PURPOSE_PASSWORD_RESET):
        raise HTTPException(status_code=400, detail="Invalid or expired verification code")
